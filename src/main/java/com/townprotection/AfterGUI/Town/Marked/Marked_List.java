...
public class Marked_List extends ListGUIAbstract<GuiManager.ListGUIPreset> {
        ...
        if(!townData.selectorMarkData.isEmpty()) {
            ...
            for (var targetItem : shows) {
                ...
                if(targetItem.getOwner() != null) {
                    var owner = Bukkit.getOfflinePlayer(targetItem.getOwner());

                    int year = targetItem.getCreationDate().getYear();
                    int month = targetItem.getCreationDate().getMonth();
                    int day = targetItem.getCreationDate().getDay();
    ...
    public ItemStack setCenterItemStack() {
        return getItem(Material.REDSTONE_BLOCK, "&a&l土地を追加する");
    }
